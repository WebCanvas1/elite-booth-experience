import { createFileRoute, Link } from "@tanstack/react-router";import { useEffect, useRef, useState } from "react";import { Check, Phone, Mail, MapPin, Camera, Sparkles, Heart, Users, ArrowRight, Star, Instagram, Facebook, ExternalLink, Calendar } from "lucide-react";import { toast } from "sonner";import heroImg from "@/assets/hero.jpg";import logo from "@/assets/logo.png";import { type Package } from "@/lib/packages";import { SiteHeader, SiteFooter } from "@/components/SiteChrome";import { Toaster } from "@/components/ui/sonner";import { useSiteContent } from "@/hooks/use-site-content";import type { ContactContent, PastEventItem } from "@/lib/site-content";import { EventVideos } from "@/components/EventVideos";

export const Route = createFileRoute("/")({head: () => ({meta: [{ title: "Elite MagicBooth — Premium Photobooth Hire Melbourne" },{name: "description",content:"Premium photobooth hire in Melbourne for weddings, birthdays, corporate events & more. Unlimited prints, custom templates, props, backdrops & digital sharing.",},{ property: "og", content: "Elite MagicBooth — Premium Photobooth Hire" },{ property: "og", content: "Stylish photobooth experiences for unforgettable events in Melbourne." },{ property: "og", content: heroImg },],}),component: Home,});

function Home() {const content = useSiteContent();

const {packages,gallery,about,contact,events,addOns,pastEvents,faqs,eventVideos,reviews,} = content;

const [selectedGallery, setSelectedGallery] = useState<PastEventItem | null>(null);const [galleryPasscode, setGalleryPasscode] = useState("");const [galleryError, setGalleryError] = useState("");

return (<div className="min-h-screen bg-background"><Toaster position="top-center" /><SiteHeader />
        ...
        {addOns.map((a) => (
          <div key={a.id} className="group relative flex flex-col rounded-3xl bg-card border border-border overflow-hidden shadow-luxe hover:-translate-y-1.5 transition-transform duration-500">
            ...
            <div className="p-3 flex flex-col flex-1">
              <h3 className="font-serif text-sm sm:text-base font-bold text-foreground mb-2">{a.title}</h3>
              <p className="text-xs sm:text-sm text-muted-foreground leading-relaxed flex-1">{a.description}</p>

              <Link
                to="/enquire"
                search={{ package: a.title }}
                className="mt-2 inline-flex items-center justify-center rounded-full gradient-gold px-3 py-1.5 text-[11px] font-bold text-ink shadow-luxe hover:scale-105 transition-transform"
              >
                Enquire Now
              </Link>
            </div>
...
function ScrollToTopButton() {const [visible, setVisible] = useState(false);

useEffect(() => {const onScroll = () => setVisible(window.scrollY > 400);
...
return (<buttontype="button"onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}className="fixed bottom-6 right-6 z-50 h-12 w-12 rounded-full gradient-gold text-ink shadow-luxe flex items-center justify-center text-2xl font-bold hover transition"aria-label="Scroll to top">↑</button>);}

function Hero({googleReviewLink,}: {googleReviewLink: string;}) {const imgRef = useRef<HTMLImageElement>(null);

useEffect(() => {const onScroll = () => {if (!imgRef.current) return;const y = Math.min(window.scrollY, 600);imgRef.current.style.transform = translate3d(0, ${y * 0.25}px, 0) scale(${1 + y * 0.0004});};window.addEventListener("scroll", onScroll, { passive: true });return () => window.removeEventListener("scroll", onScroll);}, []);

const particles = Array.from({ length: 22 }, (_, i) => {const seed = (i * 9301 + 49297) % 233280;const left = (seed / 233280) * 100;const size = 6 + ((i * 13) % 18);const delay = (i * 0.7) % 14;const duration = 14 + ((i * 5) % 12);const tx = ((i % 2 === 0 ? 1 : -1) * (20 + (i * 7) % 60));return { left, size, delay, duration, tx, key: i };});

return (<section className="relative min-h-screen flex items-center pt-24 overflow-hidden bg-ink isolate"><div className="absolute inset-0 overflow-hidden"><img
       ref={imgRef}
       src={heroImg}
     ...
     /><div className="absolute inset-0 bg-gradient-to-r from-black/85 via-black/60 to-black/30" /><div className="absolute inset-0 bg-gradient-to-b from-black/40 via-black/20 to-black/70" /><divclassName="absolute inset-0 opacity-50 mix-blend-soft-light"style={{background:"radial-gradient(60% 60% at 20% 30%, oklch(0.92 0.08 85 / 0.45), transparent 60%), radial-gradient(50% 50% at 85% 75%, oklch(0.78 0.11 80 / 0.35), transparent 65%)",}}/></div>
...
function ContactSection({ packages, contact, googleReviewLink }: { packages: Package[]; contact: ContactContent; googleReviewLink: string }) {const [submitting, setSubmitting] = useState(false);
...
return (<section id="contact" className="bg-beige text-foreground py-16 md:py-24"><div className="max-w-6xl mx-auto px-5 sm:px-6 grid lg:grid-cols-5 gap-10 md:gap-12"><div className="lg:col-span-2"><p className="text-xs uppercase tracking-[0.3em] text-gold mb-3">Get in touch</p><h2 className="font-serif text-3xl sm:text-4xl md:text-5xl mb-6 leading-tight">{contact.heading}</h2><p className="text-muted-foreground mb-8 md:mb-10 leading-relaxed">{contact.subtext}</p>
...
function Field({ label, name, type = "text", required }: { label: string; name: string; type?: string; required?: boolean }) {return (<div><label className="block text-xs uppercase tracking-widest text-muted-foreground mb-2">{label}</label><input
     name={name}
     type={type}
     required={required}
...
function SelectField({ label, name, options }: { label: string; name: string; options: string[] }) {return (<div><label className="block text-xs uppercase tracking-widest text-muted-foreground mb-2">{label}</label><select
     name={name}
     className="w-full rounded-2xl border border-input bg-background px-4 py-3 text-base sm:text-sm focus:border-gold focus:ring-2 focus:ring-gold/20 outline-none transition"
   ><option value="">Select...</option>{options.map((o) => <option key={o} value={o}>{o}</option>)}</select></div>);}
